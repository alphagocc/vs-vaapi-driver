...
uint32_t get_x_subsample(struct gbm_bo *bo, size_t plane)
{
    ...
    case GBM_FORMAT_P010: return 1;
    case GBM_FORMAT_YUV420: return 2;
    default: CHECK(false);
    }
}
...
{
    return bo->meta.strides[plane] * ALIGN(bo->meta.height, get_y_subsample(bo, plane))
...
GBM_EXPORT uint32_t gbm_bo_get_bpp(struct gbm_bo *bo)
{
    ...
    switch (bo->meta.format) {
    case GBM_FORMAT_NV12:
    case GBM_FORMAT_YUV420: return 1;
    case GBM_FORMAT_P010: return 2;
    default: CHECK(false);
    }
}
...
GBM_EXPORT int gbm_bo_get_plane_count(struct gbm_bo *bo)
{
    ...
    case GBM_FORMAT_NV12:
    case GBM_FORMAT_P010: return 2;
    case GBM_FORMAT_YUV420: return 3;
    default: CHECK(false);
    }
}
    ...
    struct gbm_device *gbm, uint32_t width, uint32_t height, uint32_t format, uint32_t flags)
{
    ...
    for (int i = 0; i < gbm_bo_get_plane_count(bo); i++) {
        ...
        bo->meta.strides[i]
            = ALIGN(width, get_x_subsample(bo, i)) / get_x_subsample(bo, i) * gbm_bo_get_bpp(bo);
    ...
    uint32_t height, uint32_t transfer_flags, uint32_t *stride, void **map_data, int plane)
{
    ...
    void *addr = mmap(
        NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, bo->meta.fds[0], bo->meta.offsets[plane]);
    CHECK(addr);
...
{
    CHECK(buffer);
    CHECK(type == GBM_BO_IMPORT_FD_MODIFIER);
