...
/**< CTB QP adjustment mode for Rate Control and Subjective Quality.
...
typedef enum csi_venc_ctbrc_mode {
	CSI_VENC_CTBRC_MODE_NONE = 0, /* JPEG use it */
	CSI_VENC_CTBRC_MODE_ONLY_SUB_QUALITY = 1,
	CSI_VENC_CTBRC_MODE_ONLY_RATE_CTRL = 2,
	CSI_VENC_CTBRC_MODE_BOTH_SUB_QUALITY_RATE_CTRL = 3,
}csi_venc_ctbrc_mode_e;

typedef struct csi_venc_rc_property {
	csi_venc_rc_mode_e rc_mode;
	csi_venc_ctbrc_mode_e ctbrc_mode;
	uint32_t qp_min;	/* 0 use default,Range:[10,51]*/
	uint32_t qp_max;	/* 0 use default,Range:[qp_min,51]*/
	bool allow_pic_skip; /* true: allow rate control to skip pictures if not enough bits are available */
	union {
		...
		csi_venc_jpeg_fixqp_s jpeg_fixqp;
	};
