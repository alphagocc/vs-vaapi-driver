...
#define CSI_VDEC_VERSION_MAJOR	1
#define CSI_VDEC_VERSION_MINOR	0
...
// FIXME: Empty struct size is different in C and C++ compiler
//        In next version, CSI HAL will add reserved fields in structures in union below
#ifndef __cplusplus
	union {
	...
	uint32_t		output_width; 	//will update if crop/scale set
	uint32_t		output_height; 	//will update if crop/scale set
	csi_vdec_output_order_e output_order;
	csi_vdec_pp_config_s pp_cfg;
...
typedef struct csi_vdec_frame_data {
	uint32_t poc;
	void *resved;
}csi_vdec_frame_data_s;
...
/* dropped this 2 function, set pp cfg using csi_vdec_set_chn_config enough
...
/*for send stream and decode  async , seperate in different thread */
int csi_vdec_send_stream_buf_async(csi_vdec_chn_t chn, csi_vdec_stream_s *stream, int32_t time_sleep);
/*for send stream and decode  async , seperate in different thread */
int csi_vdec_decode_stream_buf_async(csi_vdec_chn_t chn, int32_t timeout);
